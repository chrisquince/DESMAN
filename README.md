...
It builds upon the nextflow workflow language, which has support for cluster resource schedulers including SGE, PBS, slurm, and others.

To reconstruct strain genomes with desmanflow, you will first need to assemble your metagenome, call species bins using e.g. CONCOCT, and then provide the original metagenome assembly, the list of species contig IDs, and a directory of FastQ files as input to desmanflow.

### Installation of the desmanflow automated workflow

The following series of commands will install desman onto a clean Debian/Ubuntu system.
...
pip install --user cython numpy biopython
...
```
curl -fsSL https://cloudstor.aarnet.edu.au/plus/index.php/s/7DH5qMuM5jMbcUc > testdata.tar
tar xf testdata.tar
```
